...
from pipeline import load_compat, load_restore, process
...
def main():
    ...
    print(f"compat step={cck.get('step')}; restore step={rck.get('step') if rck else None}", flush=True)
    ...
    with zipfile.ZipFile(out_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for i, nm in enumerate(names):
            ...
            out, place, _ = process(frags, compat, restore,
