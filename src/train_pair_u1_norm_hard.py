"""PN1: photometric-invariant online hard-negative PairwiseNet training on U1 candidate rows.
...
def one_board_loss(
    ...
    # Keep raw tiles for U1 graph construction outside this function. Normalizing
    # only scorer inputs cancels independent per-tile brightness and contrast.
    tiles = photometric_normalize_tensor(tiles)
    chunks: list[Tensor] = []
    covered: list[float] = []
...
def main() -> None:
    ...
    print(f"device={device} pair_params={count_params(model):,} init_step={init.get('step')} U1=affinity{args.candidate_k}+r2x{args.r2_topk} scorer_preprocess=norm", flush=True)
