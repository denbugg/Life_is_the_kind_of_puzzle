...
def main():
 ...
 for st in range(a.steps):
  z=g[np.random.randint(len(g),size=a.batch)];bi,ai,di,slot=z.T;ci=pt[bi,di,ai];x=pairs(bt,np.repeat(bi,128),np.repeat(ai,128),ci.reshape(-1),np.repeat(di,128),dev);log=m(x).reshape(a.batch,128);loss=torch.nn.functional.cross_entropy(log,torch.from_numpy(slot).to(dev,dtype=torch.long));opt.zero_grad(set_to_none=True);loss.backward();opt.step()
