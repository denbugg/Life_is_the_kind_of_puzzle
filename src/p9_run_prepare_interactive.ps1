...
$log = Join-Path $logDir ('p9_prepare_' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.log')
try {
  Set-Location $root
  "$(Get-Date -Format o) BEGIN P9 rank96-only prepare limit=1" | Set-Content $log -Encoding UTF8
  $cmd = '"C:\Python313\python.exe" -B "src\p9_rank96_loop_g1.py" prepare --limit 1 >> "' + $log + '" 2>&1'
  & C:\Windows\System32\cmd.exe /d /s /c $cmd
  $exitCode = $LASTEXITCODE
  if ($exitCode -ne 0) { throw "P9 prepare exit code $exitCode; inspect $log" }
