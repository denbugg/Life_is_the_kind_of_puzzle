"""G2b: sparse 2x2 growing-consensus diagnostic with native R2L directions.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="G2b native-R2L 2x2 growing-consensus diagnostic")
...
def build_u1(
    ...
    return candidates, valid, r2_scores


@torch.inference_mode()
def make_directed_u1(candidates: Tensor, valid: Tensor, r2_scores: Tensor) -> tuple[Tensor, Tensor]:
    """Route each flat U1 row by its native R2L score for every cardinal direction.

    r2_scores has shape (4, tile, candidate_tile); sorting U1's same candidate
    rows independently under each channel yields a label-blind directed graph.
    """
    count, width = candidates.shape
    gathered = r2_scores.gather(2, candidates.unsqueeze(0).expand(4, -1, -1))
    direction_valid = valid.unsqueeze(0).expand(4, -1, -1)
    gathered = torch.where(direction_valid, gathered, torch.full_like(gathered, -torch.inf))
    order = gathered.argsort(dim=-1, descending=True)
    ordered_candidates = candidates.unsqueeze(0).expand(4, -1, -1).gather(2, order)
    ordered_valid = direction_valid.gather(2, order)
    return ordered_candidates.permute(1, 0, 2).contiguous(), ordered_valid.permute(1, 0, 2).contiguous()
...
def main() -> None:
    ...
    result: dict[str, object] = {
        "experiment": "G2b_native_R2L_growing_consensus",
        "images": args.n,
    ...
    for image_index in range(args.n):
        ...
        flat_candidates, flat_valid, r2_scores = build_u1(tiles, affinity, affinity2, r2, args.affinity_k, args.r2_topk, device)
        candidates, valid = make_directed_u1(flat_candidates, flat_valid, r2_scores)
