...
SRC = REPO / "src"
OUT = REPO / "kaggle_verify2"
EMBED = ["config.py", "restore_tile.py", "choose5.py",
         "verify_pair.py", "train_verify.py"]
...
TRAIN = '''# The verifier, scored on PRECISION AT THE HARVEST VOLUME.
# M456: the connected block runs 350 at edge precision 1.00, 186 at 0.99, 65 at
# 0.95 and 18 at the 0.746 we deliver, so precision at the volume harvested is
# the only number that decides anything.
import numpy as np, torch, time
from pathlib import Path

sys.argv = ["train_verify.py",
            "--dumps", str(DUMPS),
            "--held", "120",
            "--epochs", "%(epochs)d",
            "--ch", "%(ch)d", "--blocks", "%(blocks)d",
            "--strip", "8",
            "--loss", "hinge", "--margin", "%(margin)g",
            "--volume", "430",
            "--lr", "%(lr)g",
            "--seed", "0",
            "--out", "/kaggle/working/verify_hinge.pt"]
import train_verify
t0 = time.time()
train_verify.main()
...
def main():
    ...
    nb = {"cells": [cell(head + SETUP),
                    cell(TRAIN % dict(epochs=10, ch=96, blocks=5, margin=1.0,
                                      lr=3e-4))],
    ...
    (OUT / "vsos-pazzle-verify-hinge.ipynb").write_text(
        json.dumps(nb, ensure_ascii=False), encoding="utf-8")
    (OUT / "kernel-metadata.json").write_text(json.dumps({
        "id": "pasha883/vsos-pazzle-verify-hinge",
        "title": "VsOS pazzle verify hinge",
        "code_file": "vsos-pazzle-verify-hinge.ipynb",
        "language": "python",
