#!/usr/bin/env python3
"""Run the separately bound BasinCycle Stage-B MPS-transfer v2 retry.

This CLI is identical in scientific behaviour to the v1 runner.  Its only
runtime change is an installed staged MPS-float32 to CPU-float32 transfer before
the unchanged CPU-float64 deterministic proposal closure.
"""
...
DEFAULT_BINDING = (
    PROJECT_ROOT / "configs/basincycle_stage_b_execution_binding_v2.json"
)
EXECUTION_REVISION = "mps-staged-pair-logit-transfer-v2"
BASE_EXECUTION_BINDING_SCHEMA = "aiijc-basincycle-stage-b-execution-binding-v1"
EXECUTION_BINDING_SCHEMA = "aiijc-basincycle-stage-b-execution-binding-v2"
...
def _bound_path(
    ...
    *,
    ...
    if observed != expected:
        raise ValueError(f"{option} must equal the one-run bound v2 path: {expected}")
    return expected


def _validate_v2_boundary(config_path: Path, binding_path: Path) -> dict:
    if config_path != DEFAULT_CONFIG.resolve():
        raise ValueError("v2 retry requires the exact signed Stage-B scientific config path")
    if binding_path != DEFAULT_BINDING.resolve():
        raise ValueError("v2 retry requires the exact MPS-transfer v2 binding path")
    binding = load_json(binding_path)
    if binding.get("schema") != EXECUTION_BINDING_SCHEMA:
        raise ValueError("wrong Stage-B v2 execution-binding schema")
    if binding.get("execution_revision") != EXECUTION_REVISION:
        raise ValueError("wrong Stage-B v2 execution revision")
    transfer = binding.get("mps_transfer_fix", {})
    expected = {
        "failed_operation": "fused-mps-float32-to-cpu-float64",
        "replacement": "mps-float32-to-cpu-float32-then-cpu-float64",
        "scientific_semantics_changed": False,
    }
    for key, value in expected.items():
        if transfer.get(key) != value:
            raise ValueError(f"Stage-B v2 transfer binding differs at {key}")
    return binding


def _install_v2_runtime() -> None:
    current_schema = stage_b_runner.EXECUTION_BINDING_SCHEMA
    if current_schema not in {
        BASE_EXECUTION_BINDING_SCHEMA,
...
def main() -> None:
    ...
    binding = _validate_v2_boundary(config_path, binding_path)
    _install_v2_runtime()
    ...
    if output_dir.exists():
        raise FileExistsError("v2 score output directory exists; overwrite is forbidden")
