#!/usr/bin/env python3
"""Commit and measure the fixed legal DRUNet50 protected B arm on 700 boards.

This runner is deliberately not an arm-selection harness.  It renders exactly
one candidate: tilewise DRUNet50 followed by independent h20/h28/h50 NLM and
the fixed t60 h28-safe/h50-flat mask.  The same DRUNet50-h28 image is retained
only as a target-free preservation reference.  Prediction preparation never
opens a target; scoring is refused until every board is immutable and bound by
a stage commitment.
"""
...
from aiijc_puzzle.drunet_sigma50_protected_broad import (
    CANDIDATE_DRUNET50_PROTECTED as ARM_COMBINED,
)
from aiijc_puzzle.drunet_sigma50_protected_broad import (
    REFERENCE_DRUNET50_H28 as ARM_ORIGINAL_H28,
)
from aiijc_puzzle.drunet_sigma50_protected_broad import (
    image_digest,
...
CONFIG = PROJECT_ROOT / "configs/drunet_sigma50_protected_all700_measurement_v1.json"
...
def source_hashes() -> dict[str, str]:
    paths = (
        ...
        PROJECT_ROOT / "src/aiijc_puzzle/drunet_sigma50_protected_broad.py",
...
def calibration_report_allows_holdout() -> bool:
    ...
    return bool(
        report.get("schema") == "aiijc-drunet-sigma50-protected-all700-report-v1"
...
def load_context(
    ...
    if config.get("schema") != "aiijc-drunet-sigma50-protected-all700-measurement-v1":
...
def infer_board(dirty: np.ndarray, model: torch.nn.Module, device: torch.device) -> dict[str, Any]:
    ...
    reference, candidate, diagnostics = render_sigma50_protected(model, harmonized, device=device)
    if np.array_equal(reference, candidate):
        raise RuntimeError("candidate unexpectedly equals safety reference")
    return {
        "layout": layout,
        "raw": raw,
        "audit": json.loads(json.dumps(audit.as_dict(), sort_keys=True)),
...
def board_safety(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    reference_flatness = result["diagnostics"]["tile_flatness"][ARM_ORIGINAL_H28]
    candidate_flatness = result["diagnostics"]["tile_flatness"][ARM_COMBINED]
    return {
        ...
        "reference_tile_flatness": reference_flatness,
        "candidate_tile_flatness": candidate_flatness,
        "exact_constant_tile_count_increase": int(
            candidate_flatness["exact_spatially_constant_rgb_tiles"]
            - reference_flatness["exact_spatially_constant_rgb_tiles"]
        ),
        "near_flat_std_lt_2_tile_count_increase": int(
            candidate_flatness["near_flat_tiles_global_std_lt_2"]
            - reference_flatness["near_flat_tiles_global_std_lt_2"]
        ),
        "near_flat_std_lt_4_tile_count_increase": int(
            candidate_flatness["near_flat_tiles_global_std_lt_4"]
            - reference_flatness["near_flat_tiles_global_std_lt_4"]
        ),
        "candidate_pixel_distinct_from_reference": bool(not np.array_equal(candidate, reference)),
    }
...
def load_board_record(
    ...
    *,
    ...
    for key in ("safety_reference", "candidate"):
        ...
        images[key] = image
    if np.array_equal(images["candidate"], images["safety_reference"]):
        raise ValueError(f"frozen candidate equals its safety reference: {directory}")
    if verify_input_provenance:
        ...
        audit = audit_raw_permutation(dirty, raw, layout, restoration_applied_after_audit=True)
        live_audit = json.loads(json.dumps(audit.as_dict(), sort_keys=True))
        if not audit.passed or live_audit != metadata["raw_permutation_audit"]:
...
def write_board(
    ...
    try:
        ...
        for key, filename, image in (
            ("safety_reference", "R_drunet50_h28_reference.png", result["reference"]),
            ("candidate", "B_drunet50_protected_h28_h50_t60.png", result["candidate"]),
        ):
        ...
        metadata = {
            "schema": "aiijc-drunet-sigma50-protected-all700-board-v1",
            ...
            "tile_flatness": result["diagnostics"]["tile_flatness"],
...
def aggregate_safety(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    reference_exact = np.asarray(
        [row["reference_tile_flatness"]["exact_spatially_constant_rgb_tiles"] for row in rows],
        dtype=np.int64,
    )
    candidate_exact = np.asarray(
        [row["candidate_tile_flatness"]["exact_spatially_constant_rgb_tiles"] for row in rows],
        dtype=np.int64,
    )
    reference_near2 = np.asarray(
        [row["reference_tile_flatness"]["near_flat_tiles_global_std_lt_2"] for row in rows],
        dtype=np.int64,
    )
    candidate_near2 = np.asarray(
        [row["candidate_tile_flatness"]["near_flat_tiles_global_std_lt_2"] for row in rows],
        dtype=np.int64,
    )
    reference_near4 = np.asarray(
        [row["reference_tile_flatness"]["near_flat_tiles_global_std_lt_4"] for row in rows],
        dtype=np.int64,
    )
    candidate_near4 = np.asarray(
        [row["candidate_tile_flatness"]["near_flat_tiles_global_std_lt_4"] for row in rows],
        dtype=np.int64,
    )
    return {
        **summary,
        ...
        "candidate_pixel_distinct_from_reference_on_every_board": bool(
            all(row["candidate_pixel_distinct_from_reference"] for row in rows)
        ),
        "tile_flatness": {
            "reference_exact_constant_total": int(reference_exact.sum()),
            "candidate_exact_constant_total": int(candidate_exact.sum()),
            "maximum_board_exact_constant_increase": int((candidate_exact - reference_exact).max()),
            "reference_near_flat_std_lt_2_mean_max_fraction_max": [
                float(reference_near2.mean()),
                int(reference_near2.max()),
                float(reference_near2.max() / TILE_COUNT),
            ],
            "candidate_near_flat_std_lt_2_mean_max_fraction_max": [
                float(candidate_near2.mean()),
                int(candidate_near2.max()),
                float(candidate_near2.max() / TILE_COUNT),
            ],
            "near_flat_std_lt_2_delta_mean_max": [
                float((candidate_near2 - reference_near2).mean()),
                int((candidate_near2 - reference_near2).max()),
            ],
            "reference_near_flat_std_lt_4_mean_max_fraction_max": [
                float(reference_near4.mean()),
                int(reference_near4.max()),
                float(reference_near4.max() / TILE_COUNT),
            ],
            "candidate_near_flat_std_lt_4_mean_max_fraction_max": [
                float(candidate_near4.mean()),
                int(candidate_near4.max()),
                float(candidate_near4.max() / TILE_COUNT),
            ],
            "near_flat_std_lt_4_delta_mean_max": [
                float((candidate_near4 - reference_near4).mean()),
                int((candidate_near4 - reference_near4).max()),
            ],
        },
    }
...
def safety_gate(summary: Mapping[str, Any], all_provenance_pass: bool) -> dict[str, Any]:
    ...
    flatness = summary["tile_flatness"]
    checks = {
        "luma_gradient_mean_at_least_0_80": bool(
            summary["mean_luminance_gradient_retention"] >= 0.80
        ),
        "luma_gradient_min_at_least_0_70": bool(
            summary["minimum_luminance_gradient_retention"] >= 0.70
        ),
        "chroma_gradient_mean_at_least_0_80": bool(
            summary["mean_chroma_gradient_retention"] >= 0.80
        ),
        "chroma_gradient_min_at_least_0_70": bool(
            summary["minimum_chroma_gradient_retention"] >= 0.70
        ),
        "laplacian_mean_at_least_0_72": bool(summary["mean_laplacian_retention"] >= 0.72),
        "laplacian_min_at_least_0_60": bool(summary["minimum_laplacian_retention"] >= 0.60),
        "grid_ratio_mean_at_most_1_05": bool(
            summary["mean_grid_ratio_relative_to_baseline"] <= 1.05
        ),
        "grid_ratio_max_at_most_1_12": bool(
            summary["maximum_grid_ratio_relative_to_baseline"] <= 1.12
        ),
        "protected_fraction_mean_between_0_40_and_0_75": bool(0.40 <= protected[0] <= 0.75),
        "protected_fraction_every_board_between_0_30_and_0_85": bool(
            protected[1] >= 0.30 and protected[2] <= 0.85
        ),
        "clipping_increase_at_most_0_01": bool(summary["maximum_clipping_increase"] <= 0.01),
        "no_total_exact_constant_tile_increase": bool(
            flatness["candidate_exact_constant_total"] <= flatness["reference_exact_constant_total"]
        ),
        "near_flat_std_lt_2_mean_increase_at_most_2_tiles": bool(
            flatness["near_flat_std_lt_2_delta_mean_max"][0] <= 2.0
        ),
        "near_flat_std_lt_2_maximum_board_increase_at_most_6_tiles": bool(
            flatness["near_flat_std_lt_2_delta_mean_max"][1] <= 6
        ),
        "candidate_pixel_distinct_from_reference_on_every_board": bool(
            summary["candidate_pixel_distinct_from_reference_on_every_board"]
        ),
        "all_700_strict_raw_provenance_audits_pass": bool(all_provenance_pass),
    }
...
def prepare(stage: str, device: torch.device) -> None:
    ...
    commitment = {
        ...
        "purpose": "fixed_B_only_measurement_no_tuning_no_arm_selection",
        "stage": stage,
        "split": stage,
        "count": len(records),
