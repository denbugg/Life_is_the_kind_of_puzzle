#!/usr/bin/env python3
"""Freeze the signed v2 joint FIT reciprocal heads without loading labels.

The immutable FIT cache NPZ files necessarily contain ``target_slots`` because
they were produced for supervised FIT training.  The original joint runner's
generic cache loader materialises every NPZ member before dropping that field,
which is too weak for a strict pre-score target-free freeze.  This standalone
runner instead verifies the full member-name inventory, hashes the immutable
file, and materialises only an explicit non-label allowlist.  ``target_slots``
is never indexed, decoded, copied or passed to inference.

This file does not train, score, open DEV, or modify the running v2 trainer.
It can only consume the single signed v2 endpoint after FIT completes and write
the already specified fixed 5% FIT-head archive plus its pre-score freeze.
"""
...
DEFAULT_CONFIG = (
    PROJECT_ROOT / "configs/joint_reciprocal_tri_emitter_real_preregistered_v2.json"
)
DEFAULT_EXPERIMENT = (
    PROJECT_ROOT
...
LOADER_SCHEMA = "aiijc-joint-reciprocal-strict-target-free-fit-cache-loader-v1"
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.experiment_dir.resolve() != DEFAULT_EXPERIMENT.resolve():
        raise RuntimeError("strict target-free freezer is bound to the signed v2 experiment")
...
def validate_target_free_fit_cache_arrays(
    ...
    *,
    ...
    """Validate every inference-visible array without accepting a label field."""
...
def load_target_free_fit_cache(
    ...
    *,
    ...
    """Materialise only allowlisted inference arrays from one immutable NPZ.

    Inspecting the ZIP member-name inventory is permitted and necessary to fail
    closed on schema drift.  Only allowlisted members are indexed.  In
    particular, the ``target_slots`` member payload is never decoded.
    """
    ...
    with np.load(resolved, allow_pickle=False) as archive:
        if set(archive.files) != FULL_CACHE_MEMBER_NAMES:
            ...
            raise RuntimeError(
                f"FIT cache member-name inventory changed; missing={missing}, extra={extra}"
            )
...
def _verify_completed_fit(
    ...
    *,
    ...
    report_path = experiment / joint.FIT_REPORT
    endpoint = experiment / joint.FIT_ENDPOINT
    if not report_path.is_file() or not endpoint.is_file():
        raise FileNotFoundError("signed v2 FIT endpoint/report is not complete")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("schema") != "aiijc-joint-reciprocal-real-fit-report-v1":
        raise RuntimeError("signed v2 FIT report schema changed")
    if report.get("status") != "complete-single-endpoint-ready-for-target-free-dev-freeze":
        raise RuntimeError("signed v2 FIT did not reach its complete endpoint state")
    if report.get("config_sha256") != config_sha256:
        raise RuntimeError("signed v2 FIT report belongs to another config")
    if report.get("endpoint") != _record(endpoint):
        raise RuntimeError("signed v2 FIT endpoint differs from its report")
    training = report.get("training", {})
    if training.get("from_scratch") is not True or training.get("steps") != 1752:
        raise RuntimeError("signed v2 FIT training contract changed")
    return endpoint, report_path, report


def run(args: argparse.Namespace) -> dict[str, Any]:
    _validate_args(args)
    config, config_sha = joint._load_signed_config(args.config)
    if config_sha != EXPECTED_CONFIG_SHA256:
        raise RuntimeError("target-free freezer received a non-v2 signed config")
    rosters = joint.validate_source_rosters(config)
    _fit_cache_report, rows = joint._fit_cache_manifest(config, rosters)
    if len(rows) != 64:
        raise RuntimeError("strict target-free FIT roster must contain exactly 64 cases")
    ...
    device = joint._device(args.device)
    torch.use_deterministic_algorithms(
        True, warn_only=bool(args.allow_nondeterministic_mps)
    )
    model = joint._load_real_checkpoint(
    ...
    for index, row in enumerate(rows):
        cache_path = joint._project_path(row["path"])
        ...
        frozen = joint._freeze_fit_head_case(model, values, device=device)
        prefix = f"case_{index:04d}"
        all_arrays.update(
            {f"{prefix}__{key}": value for key, value in frozen.items()}
        )
        union_digest = bytes(frozen["union_identity_digest_ascii"]).decode("ascii")
    ...
    archive = experiment / joint.FIT_HEAD_ARCHIVE
    metadata = experiment / joint.FIT_HEAD_METADATA
    freeze = experiment / joint.FIT_HEAD_PRE_SCORE_FREEZE
    ...
    _write_json_exclusive(
        freeze,
        {
            ...
            "artifacts": {
                ...
                "fit_runner": _record(Path(joint.__file__).resolve()),
                "module": _record(
                    PROJECT_ROOT
    ...
    return {
        "schema": "aiijc-joint-reciprocal-strict-target-free-fit-head-freeze-result-v1",
