...
SCRIPT = ROOT / "scripts/run_taska_rawlog_tail.py"


def _load_runner() -> ModuleType:
    sys.path.insert(0, str(ROOT / "scripts"))
    specification = importlib.util.spec_from_file_location(
        "run_taska_rawlog_tail_test",
        SCRIPT,
    )
...
def test_fixed_experiment_contract() -> None:
    ...
    assert tuple(runner.PANEL_SPECS) == ("opened32", "held300", "fresh32")
...
def test_scored_layout_metric_contract_is_1104_pairs() -> None:
    ...
    metrics = runner.parent._layout_metrics(exact, exact)
    assert metrics == {
...
def test_runner_declares_target_free_freeze_before_scoring() -> None:
    source = SCRIPT.read_text(encoding="utf-8")
    assert "created_before_exact_reference_recreation" in source
    assert "candidate_objective\": \"exactly -right_log/-down_log" in source
