...
from scripts import freeze_joint_reciprocal_fit_heads_target_free_generic as freezer
...
def _write_cache(path: Path, *, extra: dict[str, np.ndarray] | None = None) -> None:
    ...
    # Successful allow_pickle=False loading proves this object label payload was
    # never indexed: materialising it would raise ValueError.
    arrays["target_slots"] = np.asarray([["secret"] * 4] * 2, dtype=object)
    arrays.update(extra or {})
    np.savez_compressed(path, **arrays)


def test_generic_loader_never_materialises_target_slots(tmp_path: Path) -> None:
...
def test_generic_loader_fails_closed_on_member_inventory_drift(
    tmp_path: Path,
) -> None:
...
def test_generic_loader_checks_whole_file_hash_before_open(tmp_path: Path) -> None:
    cache = tmp_path / "fit-cache.npz"
    _write_cache(cache)
