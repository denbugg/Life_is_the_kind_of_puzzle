...
SCRIPT = ROOT / "scripts/run_taska_adjacent_tail.py"


def _load_runner() -> ModuleType:
    sys.path.insert(0, str(ROOT / "scripts"))
    specification = importlib.util.spec_from_file_location(
        "run_taska_adjacent_tail_test",
        SCRIPT,
    )
...
def test_fixed_experiment_contract() -> None:
    ...
    assert runner.SCORED_ARMS == ("control_tail96", "adjacent_tail96")
...
def test_runner_freezes_target_free_layouts_before_scoring() -> None:
    ...
    assert "exact affected-bond delta" in source
