"""Fine-tune the relation classifier on real noisy/restored puzzle fragments.

The input-to-target permutation is a pseudo-label produced by Hungarian
matching.  It is not equally reliable for every tile, so the real-domain loss
is weighted by a bidirectional assignment confidence.  This prevents
ambiguous/repetitive tiles from dominating training with incorrect relations.
"""
...
def position_to_noisy(noisy, clean):
    ...
    assigned = cost[noisy_idx, position_idx]

    # A match is trustworthy when it is good in absolute terms and clearly
    # better than alternatives from both the noisy-tile and target-position
    # perspectives.  Robust per-image scaling keeps the score useful when the
    # amount of corruption varies between pictures.
    row_second = np.partition(cost, 1, axis=1)[:, 1]
    col_second = np.partition(cost, 1, axis=0)[1, :]
    row_margin = np.maximum(row_second[noisy_idx] - assigned, 0.0)
    col_margin = np.maximum(col_second[position_idx] - assigned, 0.0)
    scale = max(float(np.median(assigned)), 1e-3)
    absolute = np.exp(-assigned / scale)
    separation = 1.0 - np.exp(-np.minimum(row_margin, col_margin) / scale)
    pair_confidence = np.clip(absolute * separation, 0.0, 1.0)
    confidence_by_position = np.empty(len(position_idx), dtype=np.float32)
    confidence_by_position[position_idx] = pair_confidence.astype(np.float32)
    return inverse, float(assigned.mean()), confidence_by_position
...
def restore_tiles(noisy, restorer, device):
    x = torch.from_numpy(
        np.ascontiguousarray(noisy.transpose(0, 3, 1, 2))
    ).float().div_(255)
    outputs = []
    for batch in x.split(512):
        outputs.append(restorer(batch.to(device)).cpu())
    restored_tensor = torch.cat(outputs)
    restored_uint8 = (
        restored_tensor.mul(255).round().clamp(0, 255).byte()
        .permute(0, 2, 3, 1).numpy()
    )
    return restored_tensor, restored_uint8
...
def build_tile_banks(stems, data_root, restorer, device):
    ...
    for stem in tqdm(stems, desc="restore strict holdout images"):
        ...
        restored_tensor, restored_unordered = restore_tiles(noisy, restorer, device)
        inverse, cost, _ = position_to_noisy(restored_unordered, clean)
        costs.append(cost)
        ordered_raw = noisy[inverse]
        restored = restored_tensor[torch.from_numpy(inverse)]
...
@torch.inference_mode()
def build_training_assignments(pairs, restorer, device):
    """Precompute restored-domain permutations once, before DataLoader workers."""
    assignments = []
    costs = []
    confidence_means = []
    for input_path, target_path in tqdm(pairs, desc="build restored-domain assignments"):
        raw = split_tiles(load_rgb(input_path))
        clean = split_tiles(load_rgb(target_path))
        _, restored = restore_tiles(raw, restorer, device)
        inverse, cost, confidence = position_to_noisy(restored, clean)
        assignments.append((inverse, confidence))
        costs.append(cost)
        confidence_means.append(float(confidence.mean()))
    print(
        f"training_matching_cost={np.mean(costs):.6f} "
        f"training_matching_confidence={np.mean(confidence_means):.6f}"
    )
    return assignments, costs, confidence_means
...
class FineTunePairs(Dataset):
    ...
    def __init__(self, pairs, assignments):
        self.pairs = pairs
    ...
    def load_ordered(self, image_idx):
        ...
        raw = split_tiles(load_rgb(input_path))
        clean = split_tiles(load_rgb(target_path))
        inverse, confidence = self.assignments[image_idx]
        result = (raw[inverse], clean, confidence)
    ...
    def __getitem__(self, idx):
        ...
        raw, clean, confidence = self.load_ordered(image_idx)
        label = idx % 5
        rng = random
        if label == 1:
            r, c = rng.randrange(GRID), rng.randrange(1, GRID)
            ...
            while True:
                ...
                if abs(ra - rb) + abs(ca - cb) > 1:
                    break
            ia, ib = ra * GRID + ca, rb * GRID + cb
        pair_confidence = float(np.sqrt(confidence[ia] * confidence[ib]))
        pair_confidence = max(pair_confidence, MATCH_CONFIDENCE_FLOOR)
        pair_confidence = pair_confidence ** MATCH_CONFIDENCE_POWER
...
def train_epoch(model, restorer, loader, optimizer, scaler, device, epoch):
    ...
    confidence_sum = 0.0
    confidence_count = 0
    for raw_a, raw_b, clean_a, clean_b, labels, confidence in tqdm(
        loader, desc=f"fine-tune {epoch}/{EPOCHS}"
    ):
        ...
        with torch.amp.autocast("cuda"):
            restored_per_item = F.cross_entropy(
                model(restored_a, restored_b), labels, reduction="none"
            )
            restored_loss = (
                restored_per_item * confidence
    ...
    return ema, confidence_sum / max(confidence_count, 1)
...
def main():
    ...
    assignments, train_matching_costs, train_confidence_means = build_training_assignments(
        pairs, restorer, device
    )
    train_loader = DataLoader(
        FineTunePairs(pairs, assignments), batch_size=BATCH_SIZE, shuffle=False, num_workers=2,
    ...
    torch.save({
        ...
        "match_confidence_weighting": True,
    }, best_path)
    for epoch in range(1, EPOCHS + 1):
        train_loss, mean_train_confidence = train_epoch(
            relation, restorer, train_loader, optimizer, scaler, device, epoch
        )
        ...
        record = {"epoch": epoch, "train_loss_ema": train_loss,
                  "mean_train_match_confidence": mean_train_confidence,
    ...
    results["metadata"] = {
        ...
        "mean_training_hungarian_cost": float(np.mean(train_matching_costs)),
        "mean_training_assignment_confidence": float(np.mean(train_confidence_means)),
