...
def degrade_tile(tile: np.ndarray, rng, np_rng) -> np.ndarray:
    ...
    if rng.random() < 0.65:
        arr += np_rng.normal(0.0, rng.uniform(2.0, 10.0), arr.shape).astype(np.float32)
    if rng.random() < 0.45:
        arr = (arr - 127.5) * rng.uniform(0.80, 1.20) + 127.5
    if rng.random() < 0.45:
        arr += rng.uniform(-12.0, 12.0)
    arr = np.clip(arr, 0, 255).astype(np.uint8)
    if rng.random() < 0.30:
        radius = rng.uniform(0.10, 0.55)
        arr = np.asarray(Image.fromarray(arr).filter(ImageFilter.GaussianBlur(radius=radius)))
    if rng.random() < 0.30:
        arr = jpeg_roundtrip(arr, rng.randint(50, 85))
    return arr
