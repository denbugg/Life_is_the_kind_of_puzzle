...
def degrade_tile(tile: np.ndarray) -> np.ndarray:
    ...
    if random.random() < 0.45:
        ...
        arr = (arr - 127.5) * factor + 127.5
    if random.random() < 0.45:
        arr = arr + random.uniform(-16.0, 16.0)
    arr = np.clip(arr, 0, 255).astype(np.uint8)
    if random.random() < 0.35:
        arr = np.asarray(Image.fromarray(arr).filter(ImageFilter.GaussianBlur(radius=random.uniform(0.15, 0.65))))
    if random.random() < 0.35:
        arr = jpeg_roundtrip(arr, random.randint(45, 80))
    return arr
