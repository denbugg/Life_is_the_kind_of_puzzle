...
class RigidProjectionResult:
    """Rigid component packing around a frozen input-only reference layout."""

    position_to_slot: np.ndarray
    selected_proposals: int
    accepted_edges: int
    retained_accepted_edges: int
    retained_accepted_edge_fraction: float
    component_sizes: tuple[int, ...]
    rigid_components: int
    placed_component_tiles: int
    unresolved_tiles_before_assignment: int
...
def grow_components_with_edges(
    proposals: list[ProposedEdge],
) -> tuple[list[dict[int, tuple[int, int]]], list[ProposedEdge]]:
    """Grow conflict-free relative components and retain the merge witnesses."""
...
def grow_components(proposals: list[ProposedEdge]) -> tuple[list[dict[int, tuple[int, int]]], int]:
    ...
    return components, len(accepted)
...
def reference_manhattan_placement_costs(
    reference_position_to_slot: np.ndarray,
) -> np.ndarray:
    """Return position x tile distance to one frozen reference permutation.

    Distances are normalized by the maximum Manhattan distance on the 24x24
    board.  The matrix is deliberately a plain input-only unary: it contains
    no target coordinates and can be passed to the existing rigid packer and
    Hungarian completion without changing their seam terms.
    """

    reference = validate_permutation(
        reference_position_to_slot, name="reference_position_to_slot"
    )
    tile_to_position = np.empty(TILE_COUNT, dtype=np.int32)
    tile_to_position[reference] = np.arange(TILE_COUNT, dtype=np.int32)
    reference_rows = tile_to_position // GRID
    reference_columns = tile_to_position % GRID
    positions = np.arange(TILE_COUNT, dtype=np.int32)
    rows = positions // GRID
    columns = positions % GRID
    distances = (
        np.abs(rows[:, None] - reference_rows[None, :])
        + np.abs(columns[:, None] - reference_columns[None, :])
    )
    return (distances.astype(np.float32) / float(2 * (GRID - 1))).astype(
        np.float32, copy=False
    )


def _edge_is_retained(layout: np.ndarray, edge: ProposedEdge) -> bool:
    tile_to_position = np.empty(TILE_COUNT, dtype=np.int32)
    tile_to_position[layout] = np.arange(TILE_COUNT, dtype=np.int32)
    first_position = int(tile_to_position[edge.first])
    second_position = int(tile_to_position[edge.second])
...
def project_rigid_components_around_reference(
    components: list[dict[int, tuple[int, int]]],
    accepted_edges: list[ProposedEdge],
    placement_compatibility: CompatibilityMatrices,
    reference_position_to_slot: np.ndarray,
    *,
    selected_proposals: int,
    reference_weight: float = 0.5,
    beam_width: int = 8,
    beam_components: int = 16,
    translations_per_state: int = 8,
) -> RigidProjectionResult:
    """Pack relative components rigidly around a frozen global reference.

    Only whole-component translations are enumerated.  The reference layout
    contributes a normalized tilewise Manhattan unary, while external contacts
    are scored by ``placement_compatibility``.  Remaining loose tiles are
    assigned once by Hungarian; there is intentionally no post-projection QAP
    or swap refinement that could silently break the accepted component edges.
    """

    if not np.isfinite(reference_weight) or reference_weight < 0.0:
        raise ValueError("reference_weight must be finite and non-negative")
    if selected_proposals < len(accepted_edges):
        raise ValueError("selected_proposals cannot be smaller than accepted_edges")
    if beam_width <= 1 or beam_components <= 0 or translations_per_state <= 0:
        raise ValueError("rigid projection requires positive beam limits and width > 1")
    reference = validate_permutation(
        reference_position_to_slot, name="reference_position_to_slot"
    )
    ordered = sorted(
    ...
    if sum(len(component) for component in ordered) != TILE_COUNT:
        raise ValueError("components must partition all 576 tiles")
    all_tiles = [tile for component in ordered for tile in component]
    if sorted(all_tiles) != list(range(TILE_COUNT)):
        raise ValueError("components must contain every tile exactly once")

    reference_costs = reference_manhattan_placement_costs(reference)
    grid, placed_tiles = _place_components_beam(
        ordered,
        placement_compatibility,
        boundary_weight=reference_weight,
        beam_width=beam_width,
        beam_components=beam_components,
        translations_per_state=translations_per_state,
        placement_costs=reference_costs,
    )
    ...
    layout = validate_permutation(layout, name="rigid_projection_position_to_slot")
    retained = sum(_edge_is_retained(layout, edge) for edge in accepted_edges)
    accepted_count = len(accepted_edges)
    retained_fraction = (
        float(retained / accepted_count) if accepted_count else 1.0
    )
    sizes = tuple(sorted((len(component) for component in ordered), reverse=True))
    return RigidProjectionResult(
        position_to_slot=layout,
        selected_proposals=int(selected_proposals),
        accepted_edges=accepted_count,
        retained_accepted_edges=int(retained),
        retained_accepted_edge_fraction=retained_fraction,
        component_sizes=sizes,
        rigid_components=sum(size >= 2 for size in sizes),
        placed_component_tiles=int(placed_tiles),
        unresolved_tiles_before_assignment=int(unresolved),
        reference_weight=float(reference_weight),
    )


def rigid_soft_cycle_qap_projection(
    component_compatibility: CompatibilityMatrices,
    placement_compatibility: CompatibilityMatrices,
    reference_position_to_slot: np.ndarray,
    *,
    top_k: int = 8,
    ...
    reciprocal_weight: float = 0.35,
    loop_weight: float = 1.0,
    proposal_keep_fraction: float = 0.5,
    reference_weight: float = 0.5,
    beam_width: int = 8,
    beam_components: int = 16,
    translations_per_state: int = 8,
) -> RigidProjectionResult:
    """Build high-confidence soft-cycle components, then rigidly QAP-project."""
    ...
    selected = select_confident_edges(
        proposals, keep_fraction=proposal_keep_fraction
    )
    components, accepted = grow_components_with_edges(selected)
    return project_rigid_components_around_reference(
        components,
        accepted,
        placement_compatibility,
        reference_position_to_slot,
