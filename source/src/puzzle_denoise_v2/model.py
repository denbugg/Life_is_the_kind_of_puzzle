...
class TileNAFNet(nn.Module):
    ...
    def _forward_features(
        ...
        return restored, parameter_prediction, features

    def forward(self, x: torch.Tensor, return_aux: bool = False):
        restored, parameter_prediction, _ = self._forward_features(x)
        return (restored, parameter_prediction) if return_aux else restored

    def forward_with_features(
        self, x: torch.Tensor, *, return_aux: bool = False
    ):
        """Return restored tiles and the final full-resolution decoder features.

        The feature tensor is the learned ``width x 20 x 20`` representation
        immediately before the RGB residual head.  This method adds no state,
        so all existing TileNAF checkpoints remain strictly compatible.
        """

        restored, parameter_prediction, features = self._forward_features(x)
        if return_aux:
            return restored, features, parameter_prediction
