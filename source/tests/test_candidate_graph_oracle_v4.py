...
_SCRIPT = Path(__file__).resolve().parents[1] / "scripts/evaluate_candidate_graph_oracle_v4.py"
...
_VERIFIER_SPEC = importlib.util.spec_from_file_location(
    "candidate_graph_oracle_v4_verifier_for_remote_tests", _VERIFIER_SCRIPT
)
assert _VERIFIER_SPEC is not None and _VERIFIER_SPEC.loader is not None
v4_verifier = importlib.util.module_from_spec(_VERIFIER_SPEC)
sys.modules[_VERIFIER_SPEC.name] = v4_verifier
_VERIFIER_SPEC.loader.exec_module(v4_verifier)


def _valid_derivation_diagnostics() -> dict[str, object]:
    qap = {
        "objective": 1.25,
    ...
    return {
        ...
        "softcycle": {"accepted_edges": 575, "component_sizes": [576]},
        "qap": {"qap_w1": copy.deepcopy(qap), "qap_w4": copy.deepcopy(qap)},
    }


def _assert_diagnostics_lockstep(payload: dict[str, object], *, accepted: bool) -> None:
    if accepted:
        oracle._validate_derivation_diagnostics(copy.deepcopy(payload))
        v4_verifier._verify_phase_a_derivation_diagnostics(copy.deepcopy(payload))
        return
    with pytest.raises(RuntimeError):
        oracle._validate_derivation_diagnostics(copy.deepcopy(payload))
    with pytest.raises(v4_verifier.VerificationError):
        v4_verifier._verify_phase_a_derivation_diagnostics(copy.deepcopy(payload))


def test_v4_producer_verifier_nested_diagnostics_lockstep() -> None:
    _assert_diagnostics_lockstep(_valid_derivation_diagnostics(), accepted=True)
...
def test_v4_producer_verifier_nested_diagnostics_fail_closed(mutation) -> None:
    payload = _valid_derivation_diagnostics()
    mutation(payload)
    _assert_diagnostics_lockstep(payload, accepted=False)
