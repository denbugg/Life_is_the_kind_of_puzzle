...
EXPECTED_FROZEN_CONTRACT_SHA256 = (
    "5e7b8c1515c0d216e995b711cabbc59d5508518d688b80172c8a1bbe3e362ba4"
)
...
def _verify_phase_a_record(
    ...
    *,
    ...
    diagnostics = _require_object(
        record["derivation_diagnostics"], label="phase_a.derivation_diagnostics"
    )
    _require_exact_keys(diagnostics, {"hbt", "softcycle", "qap"}, label="phase_a.derivation_diagnostics")
...
def _pinned_kernel_metadata(context: ProtocolContext) -> dict[str, Any]:
    ...
    _require_exact_keys(
        launch,
        {"kernel_id", "kernel_slug", "kernel_version", "dataset_versions"},
...
def verify_phase_a_kaggle_attestation(
    ...
    *,
    ...
    wrapper_absolute = Path(wrapper_path).expanduser().absolute()
    receipt_absolute = Path(launch_receipt_path).expanduser().absolute()
...
def verify_lifecycle(
    ...
    *,
    ...
    supplied = Path(lifecycle_ledger).expanduser().absolute()
    ...
    with AnchoredRoot.open(supplied) as root:
        ...
        expected_top = {f"{state}.json" for state in LIFECYCLE_STATES} | {
            "runtime_pin_transitions"
        }
        if actual_top != expected_top:
            _fail("lifecycle ledger exact tree drift")
        code_config_sha = _verify_transition_receipts(context, ledger_root=root)
        payloads: dict[str, dict[str, Any]] = {}
        hashes: dict[str, str] = {}
    ...
    return LifecycleEvidence(supplied, payloads, hashes, code_config_sha)
...
def _post_scoring_rehash(
    ...
    *,
    ...
    if lifecycle_after.hashes != lifecycle.hashes:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config", default=str(REPO_ROOT / "configs/candidate_graph_oracle_ceiling_v3.json")
    )
