...
EXPECTED_KERNEL_SLUG = (
    "pasha883/vsos-candidate-graph-oracle-v4-phase-a-t4x2"
)
EXPECTED_KERNEL_ID = 126867315
...
RESERVATION_RECEIPT_SHA256: str | None = "4b38963476279c7ec7c6f4b17baca9997651aad4b48cf152c9165d2dca8cbf80"
EXPECTED_DATASETS = {
...
def _push_response_payload(
    ...
    if raw_ref == EXPECTED_KERNEL_SLUG:
        ref = raw_ref
    elif raw_ref == f"/code/{EXPECTED_KERNEL_SLUG}":
        # Kaggle SDK may expose the same canonical kernel reference with this
        # exact presentation prefix.  Preserve it byte-for-byte in the durable
        # raw journal, but project the validated normal receipt to the frozen
        # canonical slug.  No other spelling or path normalization is allowed.
        ref = EXPECTED_KERNEL_SLUG
    else:
...
def _validate_intent(
    ...
    *,
    ...
    if (
        ...
        or payload.get("reservation_receipt_sha256")
        != RESERVATION_RECEIPT_SHA256
...
def push_and_record(
    *,
    ...
    if (
        isinstance(EXPECTED_KERNEL_ID, bool)
        or not isinstance(EXPECTED_KERNEL_ID, int)
        or EXPECTED_KERNEL_ID <= 0
    ):
        raise RuntimeError(
            "v4 kernel reservation id is unresolved or non-positive; "
            "remote push is forbidden"
        )
    if (
        not isinstance(RESERVATION_RECEIPT_SHA256, str)
        or SHA_RE.fullmatch(RESERVATION_RECEIPT_SHA256) is None
    ):
        raise RuntimeError(
            "v4 reservation receipt hash is unresolved; remote push is forbidden"
        )
    job_dir = job_dir.expanduser().resolve(strict=True)
    receipt_path = receipt_path.expanduser().absolute()
    state_dir = (
        receipt_path.parent / "candidate_graph_oracle_v4_launch_state"
    ...
    if (
        ...
        or metadata.get("reservation_receipt_sha256")
        != RESERVATION_RECEIPT_SHA256
    ...
    if intent_path.exists() or intent_path.is_symlink():
        ...
        reservation = _validate_current_readback(
            ...
            expected_title="VSOS Candidate Graph Oracle V4 Phase A T4x2",
        )
