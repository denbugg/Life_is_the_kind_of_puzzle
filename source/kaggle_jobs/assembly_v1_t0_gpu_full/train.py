...
if not all(len(values) == 1 for values in (packages, denoisers, targets)):
    raise SystemExit(
        f"input discovery failed: packages={packages}, denoisers={denoisers}, targets={targets}"
    )
code_root = packages[0].parents[2]
data_root = targets[0].parent.parent
command = [
    sys.executable,
    str(code_root / "scripts/train_context_position.py"),
    ...
    "--feedforward-dim", "512",
    "--device", "cuda",
    "--output", "/kaggle/working/t0_gpu_full.pt",
]
...
result = {
    "schema_version": 1,
    "kind": "assembly_v1_t0_gpu_full_wrapper",
    "probe": probe,
    "seconds": time.time() - started,
    "training_report": "/kaggle/working/t0_gpu_full.json",
}
