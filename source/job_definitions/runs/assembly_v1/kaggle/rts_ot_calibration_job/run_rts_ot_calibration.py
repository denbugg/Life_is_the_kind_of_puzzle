#!/usr/bin/env python3
"""Stage robust translation synchronization calibration on Kaggle."""
...
WRAPPER = WORKING / "rts_ot_calibration_wrapper.json"
EXPECTED = {
...
def main() -> None:
    ...
    wrapper = {
        "schema_version": 1,
        "kind": "rts_ot_calibration_kaggle_wrapper",
        "status": "starting",
    ...
    write(wrapper)
    try:
        evaluator = unique(list(INPUT.rglob("scripts/evaluate_rts_ot.py")), "RTS evaluator")
        ...
        command = [
            ...
            "--irls-rounds", "8",
            "--ridge", "0.000001",
            "--output", str(WORKING / "rts_ot_calibration_report.json"),
            "--overwrite",
        ]
        wrapper.update({
            "status": "running",
        ...
        if completed.returncode:
            raise RuntimeError(f"RTS evaluator exited {completed.returncode}")
        output = WORKING / "rts_ot_calibration_report.json"
