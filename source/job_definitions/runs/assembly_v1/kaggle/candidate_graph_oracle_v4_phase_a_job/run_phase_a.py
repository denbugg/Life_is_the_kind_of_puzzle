...
CODE_SLUG = "vsos-candidate-graph-oracle-v4-code"
INPUT_SLUG = "vsos-candidate-graph-oracle-v4-inputs"
RUNTIME_SLUG = "vsos-candidate-graph-oracle-v4-runtime"
CONFIG_RELATIVE = Path("configs/candidate_graph_oracle_ceiling_v4.json")
ENV_RELATIVE = Path("configs/candidate_graph_oracle_environment_lock_v1.json")
EVALUATOR_RELATIVE = Path("scripts/evaluate_candidate_graph_oracle_v4.py")
TEST_RELATIVE = Path("tests/test_candidate_graph_oracle_v4.py")
FROZEN_SRC_RELATIVE = Path(
...
OUTPUT_ROOT = Path("/kaggle/working/candidate_graph_oracle_v4_phase_a")
WRAPPER_PATH = Path("/kaggle/working/candidate_graph_oracle_v4_phase_a_wrapper.json")
...
if isinstance(KERNEL_ID, bool) or not isinstance(KERNEL_ID, int) or KERNEL_ID <= 0:
    raise RuntimeError(
        "v4 phase-a runner has an unresolved or non-positive kernel id"
    )
if (
    not isinstance(RESERVATION_RECEIPT_SHA256, str)
    or len(RESERVATION_RECEIPT_SHA256) != 64
    or any(character not in "0123456789abcdef" for character in RESERVATION_RECEIPT_SHA256)
):
    raise RuntimeError(
        "v4 phase-a runner has an unresolved reservation receipt hash"
    )
...
base_env.update(
    {
        "PYTHONPATH": os.pathsep.join(
            [
                str(code_root / FROZEN_SRC_RELATIVE),
                str(code_root),
...
expected_launch = {
    "kernel_id": KERNEL_ID,
    "kernel_slug": f"{OWNER}/vsos-candidate-graph-oracle-v4-phase-a-t4x2",
    "kernel_version": 2,
...
if (
    ...
    or kernel_metadata.get("reservation_receipt_sha256")
