#!/usr/bin/env python3
"""Run the leakage-safe component-sync calibration exclusively on Kaggle."""
...
WRAPPER = WORKING / "component_sync_calibration_wrapper.json"
EXPECTED_ASSETS = {
...
def find_asset(name: str) -> Path:
    ...
    if name == "report.json":
        candidates = [
            ...
            if path.parent.name == "v1" and "full_union_tabular" in path.parts
        ]
...
def main() -> None:
    ...
    wrapper = {
        "schema_version": 1,
        "kind": "component_sync_calibration_kaggle_wrapper",
        "status": "starting",
    ...
    try:
        evaluator = exactly_one(
            list(INPUT_ROOT.rglob("scripts/evaluate_hgb_component_sync.py")),
            "component-sync evaluator",
        )
        ...
        command = [
            ...
            "--skip-v4",
            "--output", str(WORKING / "component_sync_calibration_report.json"),
            "--overwrite",
        ]
        wrapper.update(
            {
        ...
        output = WORKING / "component_sync_calibration_report.json"
