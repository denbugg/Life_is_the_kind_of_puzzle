...
## R5 â€” Source-Disjoint MS-SSIM U-Net Tile Restorer [stronger post-layout composition]

**Hypothesis.** R4 established that restoration improves practical rank96-layout SSIM, but its MatchDenoiser is a tiny residual CNN optimized for matching rather than the competition image metric. A substantially stronger U-Net trained on corruption-matched clean crops with MS-SSIM+L1 will recover more tile content and therefore yield a larger post-layout SSIM lift while leaving layout fixed.

**Mechanism.** Sample clean FIT target image crops in correct order; apply the existing per-tile distortion process independently to 20Ã—20 fragments; reconstruct a grid crop and predict its clean RGB target with a U-Net. Mix real train input reconstructions only through cached exact permutation labels. Optimize multi-scale SSIM plus L1. At inference, process each recovered layout canvas or its correctly placed tiles after the layout solver; never feed restored pixels into candidate mining, ranker, or buddies decisions.

**Why it is distinct.** D1 rejected MatchDenoiser for seam ranking, not final SSIM. R4 passed both oracle and frozen-rank96-layout direct SSIM gates (practical +0.05585, lower-95% +0.03681) using the tiny MatchDenoiser. The repositoryâ€™s old `train_restore.py` is an unrun/non-source-disjoint prototype: it uses an MS-SSIM+L1 RestoreNet but a legacy last-N split, and no `restore_*.pt` checkpoint exists. R5 upgrades that objective into the pinned source-disjoint protocol and evaluates direct SSIM only.

**Capacity gate.** Train on two fixed FIT scenes/crops until the restored crop score materially exceeds dirty input SSIM; reject if the stronger network cannot exceed frozen MatchDenoiser under the same corruption. Then train on FIT only and evaluate at least eight source-disjoint DEV boards, first oracle layout then frozen rank96 layout. Require a positive lower-95% confidence bound of its rank96-layout SSIM delta and a mean lift greater than R4â€™s +0.05585 under the matched protocol before retaining it over R4.

**Falsification.** Reject R5 if it does not beat MatchDenoiser on the fixed capacity control, has non-positive lower-95% practical SSIM delta, or fails to exceed R4â€™s matched practical lift. Do not train a production-scale restorer, compose outputs for submission, or run E26 before those gates pass.
