...
P35 rejected at source-disjoint G3. Continuous (row,col) regression over frozen DINO features fit FIT-train (4.215 slots MAE) but generalized poorly (6.569 slots MAE; 0.239% exact), just like P32’s absolute slot direction. Do not spend more compute on tile-only absolute-coordinate heads. Future learned placement must incorporate an image-specific relational/reconstruction signal, not a source-global semantic positional prior.

P36 G2 stopped on resource grounds, not rejected on quality: canonical solve_buddies_from_scores pairing was too slow for a 96-board gate. Before any future score-propagation experiment, introduce a cheap score-ranking/edge-recall screen or optimize the decoder; do not spend hours for an unknown delta.

P37 raw-image transformer smoke rejected: an 8-layer width-384 set transformer trained for four 96-image epochs on direct 576-way adjacency cross-entropy stayed near random (3.49% Top-20; loss 12.71). The failure is an optimization/training-budget result, not a valid test of high-capacity raw image modeling. Any successor must change the training mechanism materially (many more updates, curriculum or contrastive local objective), be preregistered independently, and retain raw RGB / source-disjoint controls.

P38 rejects the hypothesis that simply adding capacity and epochs to direct raw-RGB full 576-way adjacency cross-entropy is enough: 31M parameters and 7,680 FIT-only updates remained random (3.504% Top-20; loss 12.709). More of the same is unsupported. The next transformer must gain a nontrivial image-only pretraining objective and a denser local contrastive/continuity signal before relation fine-tuning.
