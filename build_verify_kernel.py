...
sys.argv = ["train_verify.py",
            ...
            "--strip", "4",
            "--gamma", "%(gamma)g", "--pos-weight", "%(pw)g",
            "--volume", "430",
            "--lr", "%(lr)g",
            "--seed", "0",
            "--out", "/kaggle/working/verify_pair.pt"]
import train_verify
t0 = time.time()
train_verify.main()
...
def main():
    ...
    nb = {"cells": [cell(head + SETUP),
                    cell(TRAIN % dict(epochs=8, ch=64, blocks=4, gamma=2.0,
                                      pw=2.0, lr=3e-4))],
    ...
    (OUT / "vsos-pazzle-verify.ipynb").write_text(
        json.dumps(nb, ensure_ascii=False), encoding="utf-8")
    (OUT / "kernel-metadata.json").write_text(json.dumps({
        "id": "pasha883/vsos-pazzle-verify",
        "title": "VsOS pazzle seam verifier",
        "code_file": "vsos-pazzle-verify.ipynb",
        "language": "python",
